...
import { DEFAULT_FILEUPLOAD_STATE } from "./_data/FileUploadPresets";
...
import { SECTIONS, type SectionId, type FileUploadState, type StudioPreset } from "./types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<FileUploadState>(DEFAULT_FILEUPLOAD_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const [activePresetId, setActivePresetId] = useState<string | null>(null);
  const [downloadName] = useState("file-upload-component");
  ...
  const update = <K extends keyof FileUploadState>(key: K, value: FileUploadState[K]) => {
  ...
  return (
    <AppShell contentOverflow="hidden">
      <PlaygroundLayout title="File Upload Studio" headerActions={headerActions} controls={controls} preview={output} />
