...
export default function PresetsSection({ activePresetId, onApply }: Props) {
  ...
  const families = useMemo(() => ["all", ...Array.from(new Set(FILEUPLOAD_PRESETS.map((preset) => preset.family)))], []);
  const sizes = useMemo(() => ["all", ...Array.from(new Set(FILEUPLOAD_PRESETS.map((preset) => preset.size)))], []);
  const filtered = useMemo(() => FILEUPLOAD_PRESETS.filter((preset) => {
  ...
  const surprise = () => {
    const source = filtered.length ? filtered : FILEUPLOAD_PRESETS;
